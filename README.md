...
整个过程在本地完成，无须云账号。业务引擎独立于界面，设置、事务记录和日志保存在本机应用数据目录，与仓库分离。
...
| 回收站清理 | 预览并清理仓库根目录下的 `.trash` | 仅移除明确勾选且已预览的内容 |

左侧导航固定把“仓库镜像”放在第一页。所有页面共享一个后台任务槽：镜像、统计、部署或清理不会在同一进程中重叠运行；若镜像定时任务到点时其他任务正忙，本轮会跳过，不累计补跑。只要还有部署或隔离批次等待撤销、恢复或确认，左侧会显示“待恢复事务”，并暂停所有新的写操作和定时镜像。
...
清理功能只识别真实 Obsidian 仓库根目录的 `.trash`，不会递归删除业务目录中同名的文件夹。执行前会完整列出文件、目录和大小，并要求逐仓库勾选。

正式清理先把所选内容复制到 `%LOCALAPPDATA%\ObManage` 下的隔离区并逐文件校验，确认预览未过期后才清空内容，同时保留 `.trash` 目录。页面随后可以恢复隔离内容，或再次明确确认后永久清理隔离备份；真正释放隔离备份占用的空间发生在“确认永久清理”之后。隔离区若出现未登记或变化的内容，永久清理会在删除前停止并保留现场。
...
其他管理页面的基本流程：选择仓库集合根目录和来源，扫描或分析，明确勾选目标，核对文件与目录预览及删除数量，再执行。输入、目标选择或页面上下文变化后，旧确认都会失效。
...
设置、SQLite 镜像基线、部署事务、回收站隔离备份和日志位于 `%LOCALAPPDATA%\ObManage`，并且必须与所有仓库和来源分离。即使自定义 `--state-dir` 指进仓库，程序也会在创建日志、锁或设置前拒绝。镜像的同卷临时文件、部署的同卷事务目录只在安全提交或待确认恢复期间存在；`.obmanage-deploy-*` 是保留恢复命名空间，不会进入仓库发现、统计或镜像。日志可从界面的“查看操作日志”打开。

事务记录使用独立本机密钥校验完整性并绑定恢复目录身份，用于拒绝单个记录损坏、路径漂移和孤立替换。它不是操作系统权限隔离：已经能任意读写应用状态目录和仓库的同账户程序，也能直接破坏这些数据；操作期间应关闭其他会修改目标的工具。
...
测试全部使用隔离临时目录，覆盖双向镜像、来源保护、陈旧预览、事务故障恢复、只读文件、链接边界、全局任务互斥和多尺寸界面。真实仓库的开发验收只做只读预览。
