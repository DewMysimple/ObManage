...
扫描以内容校验判断相同，覆盖所有普通文件（含视频、`.obsidian` 和 `.trash`）。仅一端存在的仓库可明确选择复制到另一端缺失的位置；如果对应位置已有未识别为仓库的目录，显示异常并拒绝覆盖。嵌套父子仓库可以分别查看，但不能在同批同时选择重叠范围。执行前复核全部所选预览，再串行更新；失败或取消就停止后续仓库，已完成部分保留，须重新扫描。扫描发现无法读取或不安全的集合内容时会明确显示并阻止整批写入。
