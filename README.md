...
ObManage 2.2 把仓库镜像、逐仓库增量处理、无视频仓库备份、仓库统计、配置与模板分发、回收站清理统一成八个独立页面；每个可能写入仓库的功能都先预览实际路径，再由用户明确选择和确认。
...
| 仓库增量处理 | 列出两端有差异的对应仓库，逐个选择电脑或移动硬盘为源 | 仅写入各仓库所选方向的目标 |
...
普通文件先暂存到目标盘，读取暂存副本完成 SHA-256 校验后再替换；新增或更新失败、取消后不会继续删除。2.1.2 在此前大量小文件优化上继续减少相邻的重复快照；异卷复制以最多一个待处理文件形成“下一文件暂存 / 当前文件写后校验”流水线，同卷仍串行避免机械盘寻道竞争。没有删除项时只做一次最终全树屏障，不重复扫描完全相同的终态；所有逐文件路径链、来源/目标快照、完整写后 SHA-256 和删除前后屏障仍保留。程序拒绝源目标相同、互相包含、目标为盘根、链接或重解析点越界，并在执行前复核路径、磁盘身份和预览是否仍有效。空来源可能清空目标时必须额外手动确认。

### 逐仓库增量处理

![逐仓库选择不同来源的工作台示例](docs/images/incremental.png)

适合“A 仓库在移动硬盘上编辑、B 仓库在电脑上编辑”的情况：选择两端仓库集合目录，扫描后只显示有差异或异常的仓库，完全一致的仓库自动隐藏。按集合内的相对路径配对，例如两端的 `学习/A` 对应；不同目录下同名仓库不会被猜测配对。可以直接选两个对应仓库根目录，此时以 `.` 配对。

每行默认“暂不处理”。你可以将 A 选为“移动硬盘为源”、B 选为“电脑为源”，分别查看两个方案的新增、覆盖、删除和完整目标位置，再勾选确认执行。数量少或时间较早的一端也可以作为来源；程序只展示差异，不推断哪一端更新。同一个仓库仍以所选来源为准，目标独有内容会删除，仅处理所选仓库范围（包含其中嵌套子仓库）；这是逐仓库选择方向，不是同一仓库内的文件冲突合并。

扫描覆盖所有普通文件（含视频、`.obsidian` 和 `.trash`）。默认快速扫描复用两端完整快照都未变化的已校验记录，首次或变化文件仍校验内容；一次比较生成双向方案，不重复读取反向内容。需要排查静默损坏或同大小、同元数据的修改时，勾选“完整内容校验”，重新读取内容；首次及完整校验仍可能耗时数分钟。页面显示本轮模式、仓库进度和总耗时。

仅一端存在的仓库可明确选择复制到另一端缺失的位置；如果对应位置已有未识别为仓库的目录，显示异常并拒绝覆盖。嵌套父子仓库可以分别查看，但不能在同批同时选择重叠范围。执行前复核全部所选预览，再串行更新；失败或取消就停止后续仓库，已完成部分保留，须重新扫描。扫描发现无法读取或不安全的集合内容时会明确显示并阻止整批写入。
