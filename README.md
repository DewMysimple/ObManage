...
普通文件先暂存到目标盘，读取暂存副本完成 SHA-256 校验后再替换；新增或更新失败、取消后不会继续删除。2.1.1 针对大量小文件合并了重复的磁盘身份检查与界面进度事件，并移除分发准备阶段中紧邻完整整树校验的重复逐文件读取，不以跳过内容校验换取速度。程序拒绝源目标相同、互相包含、目标为盘根、链接或重解析点越界，并在执行前复核路径、磁盘身份和预览是否仍有效。空来源可能清空目标时必须额外手动确认。
